...
export class AutocompleteDirective implements OnInit {
  ...
  // The directive emits ng2AutocompleteOnSelect event when an item from the listSectors is selected
  ...
  public onKey(event: any) {
    ...
    if (this.term === "" && this.listCmp) {
      // clean the listSectors if the search term is empty
      this.removeList();
    }
  }

  public ngOnInit() {
    // When an item is selected remove the listSectors
      ...
      this.search(this.term)
          .then((res) => {
            ...
            if (this.searchRequired) {
              ...
              // display the listSectors of results
              this.diplayList(res);
            }
          })
          .catch(err => {
  ...
  /**
   * Display the listSectors of results
   * Dynamically load the listSectors component if it doesn't exist yet and update the suggestions listSectors
   */
  ...
  /**
   * Update the suggestions listSectors in the listSectors component
   */
  ...
  /**
   * remove the listSectors component
   */
  private removeList() {
    this.searchInProgress = false;
    this.searchRequired = false;
